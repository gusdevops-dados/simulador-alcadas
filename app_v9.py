...
# -----------------------------
# UI de regras por cenário (com number_input)
# -----------------------------
    ...
    for col in selected:
        with st.container(border=True):
            ...
            if col in numeric_cols:
                # limites observados na base (ajudam a sugerir valores)
                ...
                if op == "≥ (mínimo)":
                    thr = st.number_input(
                        "Valor mínimo",
                        value=float(default_thr_ge),
                        step=1.0,
                        key=f"{key_prefix}_thr_ge_{col}",
                        help=f"Sugestão (dados): mín={cmin}, máx={cmax}"
                    )
                    rules.append((col, "num_ge", thr))
                elif op == "≤ (máximo)":
                    thr = st.number_input(
                        "Valor máximo",
                        value=float(default_thr_le),
                        step=1.0,
                        key=f"{key_prefix}_thr_le_{col}",
                        help=f"Sugestão (dados): mín={cmin}, máx={cmax}"
                    )
                    ...
                    with c1:
                        lo = st.number_input(
                            "Intervalo: mín",
                            value=float(default_between[0]),
                            step=1.0,
                            key=f"{key_prefix}_thr_between_lo_{col}",
                            help=f"Observado na base: {cmin} a {cmax}"
                        )
                    with c2:
                        hi = st.number_input(
                            "Intervalo: máx",
                            value=float(default_between[1]),
                            step=1.0,
                            key=f"{key_prefix}_thr_between_hi_{col}"
                        )
                    if hi < lo:
                        st.warning("⚠️ Máx não pode ser menor que mín — ajustando automaticamente.")
                        hi = lo
    ...
    with c3:
        ...
        if st.button("📄 Duplicar") and src != "(nenhum)":
            ...
            if target not in st.session_state.scenarios:
                ...
                # clona config e descarta resultado anterior
        ...
        try:
            ...
            if isinstance(cfg, dict):
                ...
                for name in cfg.keys():
                    if name not in st.session_state.scenarios:
                        st.session_state.scenarios.append(name)
                st.success("Configurações importadas! (clique 'Gerar cenário' em cada aba para processar)")
        except Exception as e:
            st.error(f"Falha ao importar: {e}")

# -----------------------------
# Render: abas de cenários (com botão GERAR)
# -----------------------------
...
for i, name in enumerate(st.session_state.scenarios):
    with tabs[i]:
        ...
        # persistir config atual (UI)
        current_cfg = {"logic": logic, "rules": rules, "order": selected_cols}
        st.session_state.scenario_cfg[name] = current_cfg

        # Botão para processar somente quando o usuário quiser
        ...
        with col_state:
            ...
            if last_run is not None:
                # verifica se config mudou desde a última execução
                if json.dumps(last_run["cfg_snapshot"], sort_keys=True, ensure_ascii=False) != json.dumps(current_cfg, sort_keys=True, ensure_ascii=False):
                    st.warning("Configuração alterada desde a última execução. Clique **Gerar/Atualizar cenário** para recomputar.", icon="⚠️")
                else:
                    st.success("Cenário está atualizado com a configuração atual.", icon="✅")
            else:
                st.info("Defina as regras e clique **Gerar/Atualizar cenário** para processar.", icon="ℹ️")
        ...
        # Se não há execução, não mostra gráficos/tabelas para não processar nada
        if name not in st.session_state.scenario_runs:
            continue

        # Usa o último resultado gerado
        out = st.session_state.scenario_runs[name]["out"]
        metrics = st.session_state.scenario_runs[name]["metrics"]
        ...
        if current_cfg["logic"].startswith("AND"):
        ...
        # Árvore simples (Matplotlib) — usa as regras atuais (cfg_snapshot)
        st.caption("🌳 Árvore de decisões — visual simples (n, % aprov., % neg. por nó).")
        max_depth = st.slider(
            "Profundidade máx.", 1, len(current_cfg["order"]) if current_cfg["order"] else 1,
            min(3, len(current_cfg["order"]) if current_cfg["order"] else 1),
            key=f"depth_{name}"
        )
        ...
        layers = build_simple_tree_layers(
            base, current_cfg["rules"], current_cfg["order"], current_cfg["logic"],
...
with tabs[-1]:
    ...
    outs = st.session_state.scenario_runs  # apenas cenários já processados
    if not outs:
